        ...
        for model_layer in model_layers:
            ...
            if layer == "input":
                ...
                if layer == 'conv1d_residual':
                    ...
                    self.network['encode_mu'] = layers.DenseLayer(self.network[self.last_layer],
                                                                  num_units=model_layer['num_units'], **self.seed)
                    self.network['encode_logsigma'] = layers.DenseLayer(self.network[self.last_layer],
                                                                        num_units=model_layer['num_units'], **self.seed)
                    self.network['Z'] = layers.VariationalSampleLayer(self.network['encode_mu'],
                                                                      self.network['encode_logsigma'])
                    self.last_layer = 'Z'
            ...
            if 'max_pool' in model_layer:
                ...
                if len(self.network[self.last_layer].output_shape) == 4:
                    if isinstance(model_layer['max_pool'], (tuple, list)):
                        ...
                        self.network[new_layer] = layers.MaxPool2DLayer(self.network[self.last_layer],
                                                                        pool_size=(model_layer['max_pool'], 1))
